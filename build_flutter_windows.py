#!/usr/bin/env python3
"""
Flutter Migration Build Helper - Windows修复版
专门为Windows系统优化的Flutter构建脚本
"""
...
class FlutterBuildHelper:
    def __init__(self):
        ...
        self.flutter_cmd = self._find_flutter_command()
        
    def _find_flutter_command(self):
        """找到正确的Flutter命令"""
        # Windows系统下尝试不同的命令格式
        if os.name == 'nt':
    ...
    def run_flutter_command(self, cmd, cwd=None):
        """运行Flutter命令"""
        if not self.flutter_cmd:
            return False, "Flutter命令未找到，请确保Flutter已安装并在PATH中"
        
        # 替换命令中的flutter为找到的正确命令
        if cmd[0] == "flutter":
            cmd[0] = self.flutter_cmd
    ...
    def check_environment(self):
        ...
        if not self.flutter_cmd:
            self.print_status("Flutter命令未找到，请确保Flutter已安装并在PATH中", "error")
            self.print_status("可以尝试手动运行: flutter --version", "info")
            return False
    ...
    def show_project_info(self):
        ...
        self.print_status(f"Flutter命令: {self.flutter_cmd or '未找到'}", "success" if self.flutter_cmd else "error")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="GitHub Follow Contributions Flutter迁移构建助手 - Windows修复版",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
  python build_flutter_windows.py                    # 构建所有平台
  python build_flutter_windows.py --platform web     # 仅构建Web
  python build_flutter_windows.py --platform android --clean  # 清理并构建Android
  python build_flutter_windows.py --debug            # 调试模式构建
  python build_flutter_windows.py --info             # 显示项目信息
        """
    )
    ...
    # 打印标题
    print("\n" + "="*60)
    print("GitHub Follow Contributions Flutter迁移构建助手 - Windows修复版")
    print("="*60)
